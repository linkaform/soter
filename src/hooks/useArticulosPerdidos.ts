...
export const useArticulosPerdidos = (location:string, area:string, status:string, enableList:boolean) => {
    ...
    const {data: listArticulosPerdidos, isLoading:isLoadingListArticulosPerdidos, error:errorListArticulosPerdidos } = useQuery<any>({
        ...
        queryFn: async () => {
            ...
            if (textMsj){
              throw new Error (`Error al obtener lista de artículos perdidos, Error: ${data.error}`);
            }else {
