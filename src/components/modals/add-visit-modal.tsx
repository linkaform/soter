...
export const AddVisitModal: React.FC<Props> = ({ title, children }) => {
  ...
  const { data: foto, isLoading: isLoadingFoto } = useUploadImage(selectedFile);

  const { data: id, isLoading: isLoadingId } = useUploadImage(selectedFileId);
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            {foto?.file_url && (
              <div className="flex flex-col justify-center items-center  my-4">
                ...
                <p className="my-3">{foto.file_name}</p>
                {/* Nombre del archivo */}
                <Image
                  src={foto.file_url}
                  alt="Imagen subida"
...
{/* 
         {id?.file_url && (
              <div className="flex flex-col justify-center items-center  my-4">
                <p className="my-3">{id.file_name}</p>
                <Image
                  src={id.file_url}
                  alt="Imagen subida"
                  width={75}
